...
def main() -> int:
    ...
    parser.add_argument("--provider", default="google", choices=["google", "ollama", "groq", "openai"])
    parser.add_argument("--model-name", default=None)
    parser.add_argument("--today", default="2026-06-01")
    parser.add_argument("--pass-threshold", type=float, default=80.0)
    parser.add_argument("--judge-provider", default=None, choices=["google", "ollama", "groq", "openai"])
    ...
    for case in cases:
        ...
        for attempt in range(5):
            try:
                ...
                if "429" in str(e) or "Rate limit" in str(e):
                    print(f"Rate limit hit for case {case['id']}. Sleeping 30s... (Attempt {attempt+1}/5)")
                    time.sleep(30)
                else:
                    raise e
        
        if raw_result is None:
            raise RuntimeError(f"Failed to process case {case['id']} after 5 retries due to rate limits.")
        ...
        # Print progress immediately to console
        print(f"[{case['id']}] Finished. Earned: {case_score.score}/{case_score.max_score}")
    ...
    try:
        ...
        sys.stdout.buffer.write(output_text.encode("utf-8") + b"\n")
